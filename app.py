...
def submit():
    ...
    c.execute('''
        INSERT INTO incidents (
            ...
            root_cause, code_review, testing, follow_up
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', data)
    conn.commit()
    conn.close()
...
def view():
    ...
    if keyword:
        count_query = '''SELECT COUNT(*) FROM incidents WHERE incident_number LIKE ? OR issue_description LIKE ? OR impact LIKE ? OR root_cause LIKE ?'''
        count_params = [f"%{keyword}%"] * 4
        c.execute(count_query, count_params)
        total_rows = c.fetchone()[0]

        query = '''SELECT * FROM incidents WHERE incident_number LIKE ? OR issue_description LIKE ? OR impact LIKE ? OR root_cause LIKE ? ORDER BY id DESC LIMIT ? OFFSET ?'''
        params = [f"%{keyword}%"] * 4 + [limit, offset]
